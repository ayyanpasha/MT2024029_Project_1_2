...
CREATE TABLE IF NOT EXISTS students (
    ...
    domain_id INT,                                  -- Foreign key for domain
    specialization_id INT,                          -- Foreign key for specialization
    placement_id INT,                               -- Foreign key for placement
    password VARCHAR(255) NOT NULL,                 -- Password field (hashed)
    CONSTRAINT fk_domain FOREIGN KEY (domain_id) REFERENCES domains(domain_id),          -- Foreign key constraint linking to 'domains'
    CONSTRAINT fk_specialization FOREIGN KEY (specialization_id) REFERENCES specialization(specialization_id),  -- Foreign key constraint linking to 'specialization'
    CONSTRAINT fk_placement FOREIGN KEY (placement_id) REFERENCES placement(id)           -- Foreign key constraint linking to 'placement'
);